...
states = iP.dump_list_comma("day7")
states = [int(x) for x in states]
states_original = states.copy()

maxi = 0
for phase in range(1234,43211): #01234 is the lowest sequence, 43210 the highest
    phases = []
    phases.append(int(phase / 10000))
    phases.append(int(phase / 1000) % 10)
    phases.append(int(phase / 100) % 10)
    phases.append(int(phase / 10) % 10)
    phases.append(phase % 10)
    
    valid = True
    for a in phases:
        if phases.count(a) > 1 or a > 4:
            valid = False
            break # We can't repeat phases or have values higher than 4
    if not valid:
        continue

    out = 0
    for i in range(0,5):
        out = calculate(states, phases[i], out)
    maxi = max(out, maxi)
